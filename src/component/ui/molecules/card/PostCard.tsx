      ...
      <div className="card-body py-6 px-2 font-medium">
        {cardData?.category
          ...
          ?.map((categoryData: any, index: number) => (
            <Link href={`/single-post/${cardData?.id}`} key={index}>
              <span
                className="btn no-animation hover:bg-primary hover:text-primary-content bg-primary/5 border-0 text-primary text-sm px-3 py-2 min-h-fit h-fit rounded-md w-fit capitalize font-work"
                key={index}
              >
                {categoryData?.name}
              </span>
